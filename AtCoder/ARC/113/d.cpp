...
signed main() {
  ...
  int N, M, K; cin >> N >> M >> K;
  init();
  vector<mint> mem_B(K+5), mem_B_sum(K+5);
  rep(b, 1, K+1) {
    mem_B[b] = modpow(mint(K-b+1), M) - modpow(mint(K-b), M);
  }

  mem_B_sum[1] = mem_B[1];
  rep(i, 2, K+3) {
    mem_B_sum[i] = mem_B[i] + mem_B_sum[i-1];
  }
  ...
  if (N == 1 && M == 1) {
    ...
    cout << modpow(mint(K), M) << endl;
  } else if (M == 1) {
    cout << modpow(mint(K), N) << endl;
  } else {
    rep(a, 1, K+1) {
      mint A = modpow(mint(a), N) - modpow(mint(a-1), N);
      mint B = mem_B_sum[K] - mem_B_sum[a-1];
      ans += A*B;
    }
    cout << ans << endl;
  }
