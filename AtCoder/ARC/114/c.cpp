...
signed main() {
  ...
  int N, M; cin >> N >> M;
  mint tot = mint(N) * modpow(mint(M), N);


  vector<vector<mint>> XW(M+1, vector<mint> (N+1));
  rep(x, 1, M+1) {
    mint mx = mint(M)-mint(x);
    XW[x][2] = mint(1);
    rep(w, 3, N+1) XW[x][w] = mx * XW[x][w-1];
  }
  vector<mint> MW(N+1); MW[N] = mint(1);
  for (int w = N-1; w >= 2; w--) {
    MW[w] = MW[w+1] * mint(M);
  }
