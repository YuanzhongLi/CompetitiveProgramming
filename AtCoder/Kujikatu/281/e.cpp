...
signed main() {
  ...
  int H, W; cin >> H >> W;
  vvi row(H), col(W);
  rep(i, 0, H) row[i].pb(-1);
  rep(i, 0, W) col[i].pb(-1);
  vvch grid(H, vch(W));
  int S = 0;
  rep(i, 0, H) {
    rep(j, 0, W) {
  ...
  rep(i, 0, H) row[i].pb(W);
  rep(i, 0, W) col[i].pb(H);
  mint ans = mint(0);
  rep(i, 0, H) {
    rep(j, 0, W) {
      if (grid[i][j] == '.') {
        int rid = upper_bound(All(row[i]), j)-row[i].begin();
        int lid = rid-1;
        int did = upper_bound(All(col[j]), i)-col[j].begin();
        int uid = did-1;
        int r = row[i][rid], l = row[i][lid], d = col[j][did], u = col[j][uid];
        int m = (r-l-1)+(d-u-1)-1;
        ans += (fact2[m]-mint(1)) * fact2[S-m];
      }
    }
  }
  cout << ans << endl;
