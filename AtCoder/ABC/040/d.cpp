...
int main() {
  ...
  int N, M; cin >> N >> M;
  vector<pair<int, pair<int, int>>> Bridge(M); // year, a, b
  rep(i, 0, M) {
    cin >> Bridge[i].second.first >> Bridge[i].second.second >> Bridge[i].first;
    Bridge[i].second.first--;
    Bridge[i].second.second--;
  }

  sort(All(Bridge)); reverse(All(Bridge));

  int Q; cin >> Q;
  vector<pair<int, pair<int, int>>> People(Q); // w, v, id
  rep(i, 0, Q) {
    cin >> People[i].second.first >> People[i].first;
    People[i].second.first--;
    People[i].second.second = i;
  }
  sort(All(People)); reverse(All(People));
  ...
  for (auto p: People) {
    int py = p.first;
    while (1) {
      if (Bridge[bid].first <= py) break;
      uf.unite(Bridge[bid].second.first, Bridge[bid].second.second);
      if (bid == M-1) break;
      bid++;
    }
    ans[p.second.second] = uf.size[uf.root(p.second.first)];
  }
