...
int main() {
  ...
  sort(All(posl)); reverse(All(posl));
  sort(All(neglabs)); reverse(All(neglabs));
  sort(All(negl)); reverse(All(negl));
  ...
  while (re) {
    if (pi+1 >= pos_num) {
      ans *= mint(neglabs[ni]*neglabs[ni+1]);
      ni+=2;
    } else if (ni+1 >= neg_num) {
      ans *= mint(posl[pi]*posl[pi+1]);
      pi+=2;
    } else {
      ll pp = posl[pi]*posl[pi+1];
      ll nn = neglabs[ni]*neglabs[ni+1];
      if (pp > nn) {
        ans *= mint(pp);
        pi+=2;
      } else {
