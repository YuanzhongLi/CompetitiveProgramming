...
mint f(int K, int types, int m) {
  mint ret = mint(0);
  rep(k, 0, K-types+1) {
    mint now = modcomb(K-types, k) * P[K-k][m];
    if (k&1) ret -= now;
    else ret += now;
  }
  ret *= modcomb(16-types, K-types);
  return ret;
};
...
signed main() {
  ...
  mint mcb15K = modcomb(15, K);
  mint mcb15K1K1 = modcomb(15, K-1) * mint(K-1);
  ...
  rep(i, 0, N) {
    ...
    if (K > 1) {
      rep(j, 0, K) {
        mint now = modcomb(K-1, j) * P[K-j][m-1];
        if (j&1) tmp0 -= now;
        else tmp0 += now;
      }
      tmp0 *= mcb15K1K1;
  ...
  rep(i, 0, N) {
    ...
    int m = N-i-1;
    int s = (i == 0 ? 1 : 0);
    int type = counter(state);
    if (type > K) break;
    int type_ = type+1;
    mint t = f(K, type, m), t_ = f(K, type_, m);
    rep(j, s, a) {
      int types = type;
      if ((state | (1ll<<j)) != state) types++;
      if (types > K) continue;
      if (m < K-types) continue;
      if (types == type) ans += t;
      else if (types == type_) ans += t_;
    }
    state |= (1ll<<a);
  }

  if (counter(state) == K) ans += mint(1);
  cout << ans << endl;
