...
int main() {
  ...
  while (!q.empty()) {
    ...
    if (Find(mp, u.first)) continue;
    mp[u.first] = u.second;
    if ((u.first&1)) {
      ll next1 = u.first+1;
      ll next2 = u.first-1;
      if (!(Find(mp, next1))) {
        q.push(make_pair(next1, u.second+1));
      }
      if (!(Find(mp, next2)) && next2 > 0) {
        q.push(make_pair(next2, u.second+1));
      }
    } else {
      ll next = u.first/2;
      if (!(Find(mp, next))) {
  ...
  ll ans = LINF;
  for (auto pi: mp) {
    chmin(ans, abs(pi.first-X)+pi.second);
    debug(pi);
  }
