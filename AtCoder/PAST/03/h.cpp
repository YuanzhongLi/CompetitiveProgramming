...
int main() {
  ...
  rep(i, 0, L+10) {
    ...
    if (V[i]) x = t3;
    if (i >= 8) {
      chmin(dp[i], dp[i-8]+2*t1+6*t2+x);
      chmin(dp[i], dp[i-4]+2*t1+2*t2+x);
      chmin(dp[i], dp[i-2]+2*t1+x);
    } else if (i >= 4) {
      chmin(dp[i], dp[i-4]+2*t1+2*t2+x);
      chmin(dp[i], dp[i-2]+2*t1+x);
    ...
    rep(j, 1, 8) {
      chmin(dp2[i+j], dp[i]+1*t1+(j-1)*t2);
    }
  }

  cout << min<ll>(dp[L], dp2[L]) << endl;
