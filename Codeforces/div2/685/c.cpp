...
bool check_range_eq(string &s, int l, int r) {
...
void solve(int tmp) {
  ...
  vi cnt(26, 0);
  rep(i, 0, n) cnt[(int)a[i]-'a']++;

  auto block = Block(b);
  auto ch_b = block.first;
  auto v_b = block.second;

  vector<pair<char, int>> bl;
  rep(i, 0, ch_b.size()) bl.pb(make_pair(ch_b[i], v_b[i]));
  sort(All(bl));
