...
signed main() {
  ...
  vector<mint> DP(N+1);
  DP[0] = mint(1); DP[1] = mint(1);
  vector<mint> DP_sum(N+1);
  DP_sum[0] = mint(1); DP_sum[1] = mint(2);
  BIT<int> bit(N);
  bit.add(A[0], 1);
  ...
  while (r < N) {
    int ar = A[r]; // A[(r+1)-1]
    int num = bit.sum_between(ar+1, N-1); // more then A[r]
    debug(r, ar, num);
    bit.show();
    ...
    while (cnt > K) {
      int al = A[l-1];
      int num2 = bit.sum_between(0, al-1); // less than A[l];
      cnt -= num2;
      bit.add(al, -1);
      l++;
    }
    ...
    if (l == r+1) {
      ...
      DP[r+1] = DP_sum[r] - (l-2 < 0 ? mint(0) : DP_sum[l-2]);
    }

    DP_sum[r+1] = DP[r+1] + DP_sum[r];
    debug(DP);
    debug(DP_sum);
    r++;
    LINE;
