...
signed main() {
  ...
  int H, W; cin >> H >> W;
  int rs, cs, rt, ct; cin >> rs >> cs >> rt >> ct; rs--; cs--; rt--; ct--;
  ...
  while (!q.empty()) {
    auto pi = q.front(); q.pop_front();
    int u = pi.first, d = pi.second;
    if (dist[u] != INF) continue;
    dist[u] = d;
    if (u == e || u == e+N) break;
    for (PI &next_pi: g[u]) {
      int v = next_pi.first, c = next_pi.second;
      if (dist[v] != INF) continue;
      if (c) {
        q.pb({v, dist[u]+c});
      } else {
        q.push_front({v, dist[u]+c});
      }
    }
  }

  cout << min(dist[e], dist[e+N]) << endl;
